...
function statusBadge(slot) {
  if (slot.isBlocked)
    return <span className="badge badge--blocked">Đã khóa</span>;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const msg =
        ...
        "Lỗi khi sinh slots. Vui lòng thử lại!";
      toast.error(msg);
    } finally {
...
export default function AdminSchedulesPage() {
  ...
  const fetchSlots = async () => {
    try {
      ...
      if (filters.isBooked !== "")
        params.isBooked = filters.isBooked === "true";
      if (filters.isBlocked !== "")
        params.isBlocked = filters.isBlocked === "true";
  ...
  const handleBlockToggle = async (id, currentBlocked) => {
    try {
      await timeSlotService.adminBlockTimeSlot(id, {
  ...
  const handleBulkDelete = async () => {
    if (
      !window.confirm(
        `Bạn có chắc chắn muốn xóa ${selected.size} slot đã chọn?`,
      )
    )
      return;
    try {
      ...
      await Promise.all(
        Array.from(selected).map((id) =>
          timeSlotService.adminDeleteTimeSlot(id),
        ),
      );
      ...
      toast.error(
        "Có lỗi xảy ra khi xóa một số slot. Có thể một số slot đã được đặt.",
      );
      fetchSlots();
    }
  };
  ...
  const pageRange = () => {
    ...
    for (
      ...
      i <= Math.min(meta.totalPages || 1, page + delta);
      i++
    )
      range.push(i);
    return range;
    ...
    <div className="schedules-page">
      ...
      <div className="slots-header">
        <div>
          ...
          <p className="slots-sub">
            Quản lý slot của bác sĩ và hỗ trợ sinh slots theo luật.
          </p>
        </div>
      ...
      {selected.size > 0 && (
        <div className="bulk-bar">
          <span>
            Đã chọn <strong>{selected.size}</strong> slot
          </span>
          <button
          ...
          <thead>
            <tr>
              <th className="col-check">
                <input
                  type="checkbox"
                  checked={selected.size === slots.length && slots.length > 0}
                  onChange={toggleAll}
                />
              </th>
              <th>Bác sĩ</th>
          ...
          <tbody>
            {isLoading ? (
              ...
              slots.map((slot) => (
                <tr
                  key={slot.id}
                  className={selected.has(slot.id) ? "row--selected" : ""}
                >
                  ...
                  <td>
                    <span className="doctor-name">
                      {formatDoctorName(slot.doctor)}
                    </span>
                  </td>
                  <td>
                    <span className="hospital-tag">
                      {slot.hospital?.name || "N/A"}
                    </span>
                  </td>
